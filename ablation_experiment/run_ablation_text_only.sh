...
OUTPUT_DIR="${1:-${COHERENCE_ROOT}/results/ablation_text_only/Qwen3.5}"
...
echo "=========================================="
echo "Ablation Experiment: text_only"
...
for bench in "${BENCH_FILES[@]}"; do
  ...
  output_file="${OUTPUT_DIR}/Qwen3.5-397B_${bench_base}_ablation_text_only.jsonl"

  echo "[RUN][text_only] benchmark=${bench}"
  echo "[RUN][text_only] output=${output_file}"
...
echo "[DONE] text_only ablation 全部完成"
